	...
	<header>
		<div class="max-width header flex-between">
			<h1 class="gradient header__author">
				<a class="home-link" href="https://www.kirillvasiltsov.com/"> Kirill Vasiltsov </a>
			</h1>
...
<style>
	...
	.mode-toggle__toggle > div > div {
		...
		background: linear-gradient(to right, var(--bg), var(--bg) 50%, var(--primary) 50%);
		border-radius: 50%;
		display: grid;
		place-items: center;
	...
	.mode-toggle__toggle > div > div > div {
		...
		height: 15px;
		border-radius: 50%;
		background: linear-gradient(to left, var(--bg), var(--bg) 50%, var(--primary) 50%);
	}
